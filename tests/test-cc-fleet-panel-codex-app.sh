...
# 组标题（主 session 名字）完整显示、绝不截断：宽屏与 RQ/完成度同行；窄分屏独占一行、RQ/完成度挪下一行；再窄就折行
assert_eq "$(printf '%s\n' "$OUT" | grep -c '^虚拟OMS上线前检查清单 .*RQ-TEST-001.*3/5 完成$')" "1" "宽屏：名字与 RQ/完成度同一行"
OUT="$(CC_FLEET_PANEL_WIDTH=40 run_panel --once --plain)"
assert_eq "$(printf '%s\n' "$OUT" | grep -c '^虚拟OMS上线前检查清单$')" "1" "窄屏：放不下时名字独占一行且完整"
assert_eq "$(printf '%s\n' "$OUT" | grep -A1 '^虚拟OMS上线前检查清单$' | tail -1 | grep -c 'RQ-TEST-001.*3/5 完成')" "1" "窄屏：RQ/完成度挪到名字下一行"
OUT="$(CC_FLEET_PANEL_WIDTH=20 run_panel --once --plain)"
assert_contains "$(printf '%s' "$OUT" | tr -d '\n')" "虚拟OMS上线前检查清单" "超窄：名字按列宽折行、拼起来完整"
assert_eq "$(printf '%s\n' "$OUT" | grep -c '^虚拟.*…')" "0" "组标题不带截断省略号"
