...
### 二之二、**跨仓库**同日都发 `-001`（2026-08-20 真实事故，与上面是两回事）

> 🚫 **RQ 是全机命名空间，不是每个仓库各一套。** 上面那条治的是「同一个仓库里手拼旧号」；这条治的是
> **两个主 session 各在自己的仓库规规矩矩跑 `cc-fleet-init`，却拿到同一个号**。

**事故经过**：`/Users/lik/ai/supply-all/qzc` 09:30 分到 `RQ-2026-0820-001`，`/Users/lik/ai/supply-agent`
10:18 也分到 `RQ-2026-0820-001` —— 两边都没手拼编号，是**序号池按仓库存**（`<git-common-dir>/fleet/.seq`）
互相看不见，各自从 001 起。但 RQ 是全机命名空间：worker session 名 `↳<模块>@<RQ>`、job 列表、fleet 面板、
`watch/status/reply/kill` 全按它认人，于是两个毫不相干的任务在同一屏里重名，人和工具都分不清谁是谁。
灌种时回查历史，这已经**重复发生过 4 次**（0601 / 0818 / 0819 / 0820），不是偶发。

**根治**（已落在脚本里）：序号池与「RQ→owner 协调目录」注册表提到跨仓库共享的 `~/.claude/fleet`
（`CC_FLEET_HOME` 可改）：
- `--alloc` 取 `max(本仓目录扫描, 本仓 .seq, 全局 .seq, 全局注册表同 stem)` + 1，**先全局锁再本仓锁**
  （顺序固定，防交叉死锁），跨仓库并发也各拿各号；
- 分配成功把 owner 仓库/协调目录写进 `~/.claude/fleet/registry/<RQ>`；
- `--fresh`（exit 4）与派发侧 `--check-join`（exit 6）据注册表识别「这个号是**别的仓库**的」并拦下；
- 全局目录不可写时**只告警不阻断**，退化成旧的「本仓唯一」，绝不让取号挂掉。

**升级到本版本后，每个用过 fleet 的老仓库各跑一次** `cc-fleet-coord --seed-global`（幂等），把历史编号灌进
全局池——否则升级后第一次分配仍会撞上别仓当天已用的号。它顺带把历史撞号打成 ⚠ 报出来。
