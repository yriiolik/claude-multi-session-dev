...
> ⛔ **任务卡不写模型 / 思考深度字段**——模型只按配置档：普通卡 `worker` 档、模式 C 打样卡 `--profile spike`，
> 都读 `~/.claude/multi-session-dev.json`，不按任务逐张判断。
...
- 协同模式: <模式 A 契约先行（并行，按契约 mock 对端）/ 模式 B 提供方先行（串行，按真实接口接入）/
  模式 C 打样先行（段① 骨架已落地 fleet/<RQ>，本卡在其上填实）/ 仅先后依赖（无新接口）/ 无>
- 上游依赖: <依赖哪个模块的什么——契约文件 `<COORD>/contracts/<name>.contract.md`、或上游模块回执里
  的真实接口形态（模式 B）、或 fleet/<RQ> 上的骨架：接口签名文件 + 段① 集成 e2e 路径（模式 C）；无则写"无">
...
## 接口/契约约束（如涉及跨模块——协同三模式，见 reference/contract-first.md）
...
**模式 C · 打样先行：**
- **打样卡（段①，唯一一张跨模块卡，派发加 `--profile spike`）**：产出 ①契约文件 `<COORD>/contracts/<name>.contract.md`
  （过字段级核对表）②每个相关模块的接口签名 / 类型 / 空实现（函数体只许抛「未实现」或返回固定值）③默认关闭的接线
  开关（唯一一处赋值，默认 = 改造前行为）④一条真跨模块集成 e2e（最小假数据打到底，路径写进回执）。⛔ 不写业务逻辑；
  改动统计里业务文件行数明显超出骨架量级 = 越线，主 session 退回重做。
- **填实卡（段②，一模块一张）**：依赖锚点 = `fleet/<RQ>` 上的骨架（接口签名文件 + 段① 集成 e2e 路径）；只填函数体、
  只动本模块；不改签名——签名要改 → 写「需主 session 裁决」。自测须跑段① 那条集成 e2e（跑前 `cc-fleet-e2e-lock acquire`）。
...
- 自测结果: <单测/e2e pass 条数>
- 测试结果文件: <落盘路径，如 test-results/<module>.json / playwright-report/results.json；主 session 只读文件不看控制台>
