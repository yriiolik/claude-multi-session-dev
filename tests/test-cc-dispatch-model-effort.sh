...
echo "[8] --profile spike：「打样先行」段① 骨架 worker 走独立配置档，内置默认 fable5.1 + xhigh"
rm -f "$CFG"
eq "无配置文件 → spike 内置 claude-fable-5-1 + xhigh" "$(dump respawn --profile spike)" "--model claude-fable-5-1 --effort xhigh"
...
eq "配置文件只有 worker 块时 spike 用内置默认"    "$(printf '{"worker":{"model":"claude-sonnet-5","effort":"low"}}' > "$CFG"; dump respawn --profile spike)" "--model claude-fable-5-1 --effort xhigh"
eq "--effort 命令行仍可临时覆盖 spike 档"         "$(dump respawn --profile spike --effort high)" "--model claude-fable-5-1 --effort high"
